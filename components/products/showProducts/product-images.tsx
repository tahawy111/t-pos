...
const ProductImages: FC<ProductImagesProps> = ({ images }) => {
  ...
  return (
    <div className="flex justify-center items-center flex-col p-5">
      {images.length === 0 ? (
        <h1 className="text-2xl">No Images Available</h1>
      ) : (
            ...
            >
              {images.map((img, index) => (
                <Image
                  ...
                  className={cn(
                    "w-32 h-32 object-contain rounded-sm cursor-pointer border-2 mx-1",
                    index === imgIndex && "border-4 border-blue-500/30"
                  )}
                  alt={`Thumbnail ${index}`}
                  width={500}
                  height={500}
